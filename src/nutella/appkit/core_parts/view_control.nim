...
proc stringValue*(control: NSControl): string =
  if control.isNil:
    return ""
  if control.isKindOfClass(NSTextField):
    var field = asType[NSTextField](control.value)
    result = field.stringValue()
    field.value = nil
    return
  if control.isKindOfClass(NSButton):
    var button = asType[NSButton](control.value)
    result = button.title()
    button.value = nil
    return
  ""
...
proc setStringValue*(control: NSControl, value: string) =
  ...
  if control.isKindOfClass(NSTextField):
    ...
    field.value = nil
  elif control.isKindOfClass(NSButton):
    var button = asType[NSButton](control.value)
    button.setTitle(value)
    button.value = nil

proc intValue*(control: NSControl): cint =
  if control.isNil:
    return 0.cint
  if control.isKindOfClass(NSButton):
    var button = asType[NSButton](control.value)
    result = button.state().cint
    button.value = nil
    return
  try:
    parseInt(control.stringValue()).cint
  except ValueError:
    0.cint

proc integerValue*(control: NSControl): int =
  control.intValue().int

proc floatValue*(control: NSControl): cfloat =
  if control.isNil:
    return 0.0
  if control.isKindOfClass(NSButton):
    var button = asType[NSButton](control.value)
    result = button.state().cfloat
    button.value = nil
    return
  try:
    parseFloat(control.stringValue()).cfloat
  except ValueError:
    0.0

proc doubleValue*(control: NSControl): cdouble =
  if control.isNil:
    return 0.0
  if control.isKindOfClass(NSButton):
    var button = asType[NSButton](control.value)
    result = button.state().cdouble
    button.value = nil
    return
  try:
    parseFloat(control.stringValue()).cdouble
  except ValueError:
    0.0

proc setIntValue*(control: NSControl, value: cint) =
  if control.isNil:
    return
  if control.isKindOfClass(NSButton):
    var button = asType[NSButton](control.value)
...
proc setIntegerValue*(control: NSControl, value: int) =
  control.setIntValue(value.cint)

proc setFloatValue*(control: NSControl, value: cfloat) =
  if control.isNil:
    return
  if control.isKindOfClass(NSButton):
    var button = asType[NSButton](control.value)
    button.setState(value.int.cint)
    button.value = nil
  else:
    control.setStringValue($value)

proc setDoubleValue*(control: NSControl, value: cdouble) =
  if control.isNil:
    return
  if control.isKindOfClass(NSButton):
    var button = asType[NSButton](control.value)
...
proc takeStringValueFrom*(control: NSControl, sender: NSControl) =
  if control.isNil or sender.isNil:
    return
  control.setStringValue(sender.stringValue())

proc takeIntValueFrom*(control: NSControl, sender: NSControl) =
  if control.isNil or sender.isNil:
    return
  control.setIntValue(sender.intValue())

proc takeIntegerValueFrom*(control: NSControl, sender: NSControl) =
  if control.isNil or sender.isNil:
    return
  control.setIntegerValue(sender.integerValue())

proc takeFloatValueFrom*(control: NSControl, sender: NSControl) =
  if control.isNil or sender.isNil:
    return
  control.setFloatValue(sender.floatValue())

proc takeDoubleValueFrom*(control: NSControl, sender: NSControl) =
  if control.isNil or sender.isNil:
    return
  control.setDoubleValue(sender.doubleValue())

proc performClick*(control: NSControl, sender: NSObject) =
  if control.isNil:
    return
  if control.isKindOfClass(NSButton):
    var button = asType[NSButton](control.value)
