...
proc dumpComboBoxLine(comboBox: NSComboBox, name: string) =
  if comboBox.isNil:
    return
  dumpViewLine(asRetainedType[NSView](comboBox), name)
  echo "[",
    name,
    "] editable=",
...
objcImpl:
  ...
  method init*(self: var ComboBoxWindow): ComboBoxWindow =
    ...
    dumpComboBoxLayout(
      "init", asRetainedType[NSWindow](result), result.xComboBox1, result.xComboBox2
    )
...
when isMainModule:
  ...
  if getEnv("NUTELLA_COMBOBOX_TRIGGER_CALLBACK").strip().len > 0:
    ...
    window.OnComboBox1SelectedItemChange(asRetainedType[NSObject](window))
