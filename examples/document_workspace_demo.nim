...
proc makeWorkspaceWindow(
  ...
  menuBar[anHeight].equalTo(28.0).active = true
  title.pinEdges(
    toGuide = root.contentLayoutGuide(initEdgeInsets(46.0, 26.0, 0.0, 26.0)),
    edges = {leLeft, leTop, leRight},
  )
  activate(
    cx(title[anHeight] == 30.0),
    cx(subtitle[anTop] == title[anBottom] + 4.0),
    cx(subtitle[anLeft] == title[anLeft]),
    cx(subtitle[anRight] == title[anRight]),
    cx(subtitle[anHeight] == 20.0),
    cx(toolbar[anTop] == subtitle[anBottom] + 14.0),
    cx(toolbar[anLeft] == title[anLeft]),
    cx(toolbar[anRight] == title[anRight]),
    cx(toolbar[anHeight] == 30.0),
    cx(table[anTop] == toolbar[anBottom] + 18.0),
    cx(table[anLeft] == title[anLeft]),
    cx(table[anRight] == sideTitle[anLeft] - 22.0),
    cx(table[anBottom] == root[anBottom] - 26.0),
    cx(sideTitle[anTop] == table[anTop] + 4.0),
    cx(sideTitle[anRight] == title[anRight]),
    cx(sideTitle[anWidth] == 260.0),
    cx(sideTitle[anHeight] == 22.0),
    cx(activity[anTop] == sideTitle[anBottom] + 10.0),
    cx(activity[anLeft] == sideTitle[anLeft]),
    cx(activity[anRight] == sideTitle[anRight]),
    cx(activity[anHeight] == pasteboardInfo[anHeight]),
    cx(pasteboardTitle[anTop] == activity[anBottom] + 24.0),
    cx(pasteboardTitle[anLeft] == sideTitle[anLeft]),
    cx(pasteboardTitle[anRight] == sideTitle[anRight]),
    cx(pasteboardTitle[anHeight] == sideTitle[anHeight]),
    cx(pasteboardInfo[anTop] == pasteboardTitle[anBottom] + 10.0),
    cx(pasteboardInfo[anLeft] == sideTitle[anLeft]),
    cx(pasteboardInfo[anRight] == sideTitle[anRight]),
    cx(pasteboardInfo[anHeight] == accessibilityInfo[anHeight]),
    cx(accessibilityTitle[anTop] == pasteboardInfo[anBottom] + 24.0),
    cx(accessibilityTitle[anLeft] == sideTitle[anLeft]),
    cx(accessibilityTitle[anRight] == sideTitle[anRight]),
    cx(accessibilityTitle[anHeight] == sideTitle[anHeight]),
    cx(accessibilityInfo[anTop] == accessibilityTitle[anBottom] + 10.0),
    cx(accessibilityInfo[anLeft] == sideTitle[anLeft]),
    cx(accessibilityInfo[anRight] == sideTitle[anRight]),
    cx(accessibilityInfo[anBottom] == table[anBottom]),
  )
...
proc makeTextDocumentWindow(
  ...
  activate(
    ...
    cx(subtitle[anRight] == title[anRight]),
    cx(subtitle[anHeight] == 20.0),
    cx(scroll[anTop] == subtitle[anBottom] + 18.0),
    cx(scroll[anLeft] == title[anLeft]),
    cx(scroll[anRight] == title[anRight]),
    cx(scroll[anBottom] == root[anBottom] - 24.0),
  )
