...
proc closeFeatureDocuments(controller: FeatureDocumentController) =
  if controller.isNil or controller.closingDocuments:
    return
  controller.closingDocuments = true
  defer:
    controller.closingDocuments = false
  var documents: seq[Document]
  for document in controller.documents:
    if not document.isNil:
      documents.add document
  for document in documents:
    discard controller.closeDocument(document, reviewUnsaved = false)

protocol FeatureDocumentWindowLifecycle of WindowDelegateProtocol:
  method windowDidClose(controller: FeatureDocumentController, window: Window) =
    if controller.primaryWindow == window:
      controller.closeFeatureDocuments()
...
protocol FeatureDocWindows of DocumentWindowProtocol:
  method makeWindowControllers(document: FeatureDocument): seq[WindowController] =
    ...
    let window = document.makeDocumentWindow(controller, app)
    if controller.primaryWindow.isNil:
      controller.primaryWindow = window
    window.delegate = controller
    @[newWindowController(window)]
...
proc makeFeatureDocumentController(app: Application): FeatureDocumentController =
  ...
  discard result.withProtocol(FeatureDocumentWindowLifecycle)
