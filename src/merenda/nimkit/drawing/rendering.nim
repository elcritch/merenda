...
proc takeOnlyRootChildren(
    source: var RenderList, sourceRoot: FigIdx, shell: var Fig, contents: var RenderList
) =
...
proc collectRenderFrames(
  ...
  if capture:
    frames.add scene.captureViewFrame(view, viewId, parentViewId, cacheKey, appearance)
  else:
...
proc updateRenderScene(root: View, appearance: Appearance): RenderScene =
  ...
  if not root.xCachedRenderScene.isNil and root.xCachedRenderScene.frameGeneration() > 0 and
