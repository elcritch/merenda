...
suite "nimkit grid views":
  test "intrinsic size uses max row and column tracks with spacing and insets":
    let
      grid = newGridView(frame = rect(0, 0, 1, 1))
    ...
    check second.frame() == rect(47.0, 1.0, 60.0, 20.0)
    check third.frame() == rect(2.0, 28.0, 40.0, 30.0)
    check fourth.frame() == rect(47.0, 28.0, 60.0, 30.0)
  ...
  test "spanning subviews grow affected tracks":
    ...
    check header.frame() == rect(0.0, 0.0, 120.0, 20.0)
    check left.frame() == rect(0.0, 25.0, 50.0, 10.0)
    check right.frame() == rect(60.0, 25.0, 60.0, 10.0)

  test "directional alignment can center and trail inside filled tracks":
    let
      grid = newGridView(frame = rect(0, 0, 200, 80))
    ...
    check first.frame() == rect(40.0, 70.0, 20.0, 10.0)
    check second.frame() == rect(140.0, 60.0, 20.0, 20.0)
  ...
  test "hidden grid subviews are omitted from intrinsic size and layout":
    ...
    grid.sizeToFit()
    grid.layoutSubtreeIfNeeded()
    check visible.frame() == rect(0.0, 0.0, 40.0, 20.0)
    check hidden.frame() == rect(0.0, 0.0, 100.0, 50.0)

  test "grid content changes invalidate grid and parent lazily":
    let
      root = newView(frame = rect(0, 0, 300, 120))
      grid = newGridView(frame = rect(10, 10, 1, 1))
  ...
  test "grid participates in solver constraint layout":
    ...
    check grid.frame() == rect(20.0, 10.0, 250.0, 40.0)
  ...
  test "removing a grid subview removes its placement":
    let
      grid = newGridView(frame = rect(0, 0, 100, 20))
      first = newFixedIntrinsicView(20, 10)
    ...
    grid.layoutSubtreeIfNeeded()
    check second.frame() == rect(44.0, 0.0, 56.0, 20.0)
